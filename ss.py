...
def price_pred(crypto_name,crypto_currnecy,*args):
    def main():
        ...
        global cplusres
        cplusres=csoup.find_all("span",class_="live-percent-change tw-ml-2 tw-text-xl")
        cplusres=str(cplusres)
        cplusres=cplusres.split("class=")
        cplusres=cplusres[2].split()
        cplusres=cplusres[0].replace('"','')
        if cplusres=="text-green":
            cplusres="+"
        else:
            cplusres="-"
        sscurcpulsers=csoup.find_all('td', class_="text-center")
        aacurcpulsers=sscurcpulsers[1].text.strip('\n')
        print(aacurcpulsers)
        ...
        for line in args:
            ...
            plt.title(f'{crypto_currnecy} price pred')
            plt.xlabel('Time')
            plt.ylabel('Price')
            ...
            prediction=prediction.replace(']]','')
            cyptonameslist.append(crypto_name)
            print(cres[0].text)
            global crypt_name
            ...
            if line==args[::-1][0]:
                ...
                for lines in range(len(predictionlist)):
                    ...
                    exec(x)
            if ne:
                driver.get(f'https://www.xe.com/currencyconverter/convert/?Amount={prediction}&From=USD&To=EUR')
                time.sleep(3)
...
def find_val(crypt_names):
    x=f'crypt_names_compred={crypt_names}_compred'
    exec(x)
